...
const VideoCallScreen: React.FC = () => {
  ...
  const startCall = async () => {
    try {
      ...
      if (!constellationId) {
        Alert.alert('No Pair Yet', 'Video call is available when both partners are connected.');
        return;
      }

      const id = await databaseBackedCallAdapter.startVideoSession(constellationId);
      setSessionId(id);
      setStatus('ringing');
    } catch {
  ...
  return (
    <Screen showHeader headerTitle="Video Call">
      <View style={styles.container}>
        <Text style={styles.title}>Private 1:1 Video Space</Text>
        ...
        {status === 'idle' && (
          <TouchableOpacity style={styles.primaryButton} onPress={startCall}>
            <Text style={styles.primaryText}>Start Video Call</Text>
          </TouchableOpacity>
        )}
