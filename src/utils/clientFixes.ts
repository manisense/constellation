...
import { enqueuePairNotification, supabase } from "./supabase";
...
export const uploadImage = async (
  ...
  try {
    ...
    return supabase.storage.from("chat-images").getPublicUrl(filePath).data
      .publicUrl;
  } catch (error) {
...
export const setupMessageSubscription = (
  ...
  console.log(
    `Setting up real-time subscription for messages in constellation: ${constellationId}`
  );
 ...
 */
export const getConstellationMessages = async (
  constellationId: string
): Promise<Message[]> => {
  try {
...
export const getPartnerProfile = async (
  ...
  try {
    ...
    try {
      ...
      console.log(
        "Partner profile retrieved successfully via RPC:",
        data.partner
      );
      return data.partner;
    } catch (rpcError) {
      ...
      console.log(
        "Partner profile retrieved successfully via direct query:",
        partnerProfile
      );
...
export const getConstellationData = async (
  ...
  try {
    ...
    const { data: bondingData, error: bondingError } = await supabase.rpc(
      "get_bonding_strength",
      {
        constellation_id: constellationId,
      }
    );
    ...
    const combinedData = {
      ...constellationData,
      bonding_strength:
        bondingData?.bonding_strength ||
        constellationData.bonding_strength ||
        0,
    };
...
export const increaseBondingStrength = async (
  ...
  try {
    console.log(
      `Increasing bonding strength for constellation ${constellationId} by ${amount}`
    );
